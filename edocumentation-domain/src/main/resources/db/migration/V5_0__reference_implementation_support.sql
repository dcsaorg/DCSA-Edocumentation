...
CREATE TABLE assigned_equipment (
    id uuid PRIMARY KEY,
    confirmed_booking_id uuid NOT NULL REFERENCES confirmed-booking(id),
...
ALTER TABLE confirmed-booking ADD valid_until timestamp with time zone NULL;
CREATE UNIQUE INDEX unq_valid_until_shipment_idx ON confirmed-booking(carrier_booking_reference) WHERE valid_until IS NULL;
